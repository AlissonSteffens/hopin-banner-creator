...
function loadJSON(json) {
  ...
  let altColor = data.altColor ? data.altColor : "#000000";
  let textColor = data.textColor ? data.textColor : "#ffffff";
  let backgroundColor = data.backgroundColor ? data.backgroundColor : "#ffffff";
  // set the altColor as css variable
  document.documentElement.style.setProperty("--altColor", altColor);
  // set the textColor as css variable
  document.documentElement.style.setProperty("--textColor", textColor);
  // set the backgroundColor as css variable
  ...
  sections.forEach((section) => {
    ...
    values.forEach((value) => {
      ...
      if (value.indexOf(":") !== -1) {
        let sec = value.split(":");
        if (
          sec[0].toLowerCase().trim() === "palestra" ||
          sec[0].toLowerCase().trim() === "painel" ||
          sec[0].toLowerCase().trim() === "st" ||
          sec[0].toLowerCase().trim() === "session"
        ) {
          sec.shift();
        }
        main += `${sec[0]} `;
      ...
      if (section.people) {
        section.people.forEach((person) => {
          ...
          if (person.headline && person.headline != "") {
            ...
            if (
              !(section.people.length - 1 == section.people.indexOf(person))
            ) {
              people += ", ";
            }
          }
...
function save() {
  for (let i = 0; i < sections.length; i++) {
    html2canvas(document.getElementById(i)).then((canvas) => {
      ...
      link.setAttribute(
        "download",
        `${sections[i].event}/${sections[i].section}.png`
      );
      link.setAttribute(
        "href",
